...
ExpressionCheck::ExpressionCheck() {
    left_prime_phrase = "";
    is_move = true;
    error_info.push_back("缺少表达式");
    error_info.push_back("表达式间无运算符");
    error_info.push_back("括号间无表达式");
    error_info.push_back("非法左括号");
    error_info.push_back("表达式间缺少运算符");
...
int ExpressionCheck::check_error(string left_prime_phrase, char normalize_char) {
    ...
    if (normalize_char == 'i') { //表达式两侧是否存在运算符
        ...
        if (index != left_prime_phrase.size() - 1 != 0 && left_prime_phrase[index + 1] == 'i') { //表达式左侧是缺少运算符
    ...
    if (normalize_char == '(' || normalize_char == ')') { //括号合法性检查
        ...
        if (left_index != -1 && right_index != -1) { //括号间是否存在表达式
        ...
        if (left_index + 1 == right_index) { //缺少运算符
            return DEFECT_OPERATOR;
        }
    ...
    if (terminal_symbols.find(normalize_char) != terminal_symbols.end()) { //运算符间是否存在非终结符
        int index = left_prime_phrase.find(normalize_char);
        if ((index != 0 && (terminal_symbols.find(index - 1) != terminal_symbols.end()))) //运算符左侧缺少运算符
            return EXPRESSION_ERROR;
        if ((index != left_prime_phrase.size() - 1 && (terminal_symbols.find(index + 1) != terminal_symbols.end()))) //运算符右侧缺少运算符
            return EXPRESSION_ERROR;
    }
    return OK;
}
    ...
    while ((table[make_pair(p, a)] == '>' || a == '#') && !symbols_stack.empty()) { //栈顶优先级高于输入串首
        ...
        if (table[make_pair(top_value, q)] == '=' || table[make_pair(top_value, q)] == '<' || top_value == '#') { //到达最左素短语的最后一个终结符
            ...
            if (match_production_left(left_prime_phrase)) { //是否满足规约的条件
                ...
                if (error_type != OK) {
                    //cout << error_info[erro_type] << endl;
...
int ExpressionCheck::check_expression(string expression) {
    ...
    while (expression[index] != '#') { //遍历表达式串
        ...
        //if(table[make_pair(top_value, a)] == '<' || table[make_pair(top_value, a)] == '=')
        //
        //if((table[make_pair(top_value, a)] == '>')) { //规约前的记录
        //    save_analysis_step(index, table[make_pair(top_value, a)], left_phrase, "规约");
        //}
        top_value = normalization(q, a); //规约操作
        if (!left_phrase.empty()) { //规约
            save_analysis_step(index, table[make_pair(top_value, a)], left_phrase, "规约");
            left_phrase.clear();
        }
    ...
    while (expression[index] == '#' && symbols_stack.size() > 2) {
        ...
        save_analysis_step(index, '>', left_phrase, "规约");
